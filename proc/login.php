...
if (isset($_POST["username"]) && isset($_POST["password"])) {
    ...
    $sql = "SELECT usuario_cam, password_cam FROM camarero WHERE usuario_cam = ?";
    $stmt = mysqli_prepare($conn, $sql); // Preparar la consulta SQL
    mysqli_stmt_bind_param($stmt, "s", $username); // Vincular el nombre de usuario a la consulta
    ...
    if ($row = mysqli_fetch_assoc($result)) {
        ...
        $nombre = $row['usuario_cam'];
        $password = $row['password_cam'];
